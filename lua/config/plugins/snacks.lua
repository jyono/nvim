...
return {
  {
    ...
    config = function(_, opts)
      ...
      vim.keymap.set('n', '<leader>sD', function()
        picker.diagnostics { cwd = vim.fn.expand '%:p:h', title = 'Diagnostics in current directory' }
      ...
      vim.keymap.set('n', '<leader>sag', function()
        picker.grep { title = 'Live Grep (All Files)', hidden = true, ignored = true, exclude = { '.git/' } }
      end, { desc = '[S]earch [A]ll Files [G]rep' })
      vim.keymap.set('n', '<leader>saf', function()
        picker.files { title = 'Find All Files (Hidden + Ignored)', hidden = true, ignored = true, exclude = { '.git/' } }
